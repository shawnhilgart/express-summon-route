...
summon.use = function(expressApp, exp) {
    router = expressApp._router;
    app = expressApp;
    express = exp; // pass reference of version of express, changes in req/res proto's caused issues

    summon.protoSetup(exp);

};


summon.protoSetup = function(exp) {


    Request.prototype = exp.request; // add on the additional express tools each request inherits

    Response.prototype = exp.response; // add on the additional express tools each response inherits
